...
async def send_welcome(message: types.Message):
    await message.reply("Здравствуйте! Отправьте мне сообщение, и ждите публикации, или ответа от администрации!")
...
async def echo_text_message(message: types.Message):
    ...
    await bot.send_message(
        ...
        text=f'Сообщение от {user.first_name} (@{user.username}): {message_text}'
    )
    await bot.send_message(user.id, "Cпасибо за сообщение, ждите ответа!")
...
async def echo_photo_with_caption(message: types.Message):
    ...
    await bot.send_message(user.id, "Cпасибо за сообщение, ждите публикации!")
...
async def echo_video_message(message: types.Message):
    ...
    await bot.send_video(
        chat_id=ADMIN_CHAT_ID,
        video=video,
        caption=f'Видео от {user.first_name} (@{user.username}): {caption}'
    )
    await bot.send_message(user.id, "Cпасибо за сообщение, ждите публикации!")
...
async def echo_media_group_message(message: types.Message):
    ...
    media[0].caption = f'Сообщение від {user.first_name} (@{user.username})'
